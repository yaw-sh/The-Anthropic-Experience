...
| `ReviewandResearchReport.md` | ChatGPT conversation export (full version — canonical; replaces the earlier partial export below at the operator's instruction): the Pro session that produced both packs, the roast correction, the master blueprint, its four-part GitHub failure (`3e1f625`/`3119913` on `main`), and its closing hierarchy ruling — root blueprint canonical, pack supporting, old Claude plan "superseded" (ChatGPT's recommendation; recorded as an owner decision in `anthropic_experience_review_pack/OWNER-DECISIONS.md`, row 1). |
| `ReviewandResearchReport_1.md` | The earlier, partial export of the same Pro session. Superseded by the file above but **restored, not deleted** — the operator's explicit ruling is that originals are never removed from the repo, only superseded. It was git-removed once during the replacement and restored from history (`5a4d8bb`) in the same session that caught the omission. |
